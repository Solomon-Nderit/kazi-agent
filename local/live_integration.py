...
def execute_pc_action(action: Literal['click', 'click_and_type', 'press_key'], target: str = "", value: str = "") -> dict:
    """Executes a PC automation action on the user's screen.
    
    Args:
        action: The type of action to perform. Must be strictly one of: 'click', 'click_and_type', 'press_key'.
        target: The normalized y, x coordinates (e.g., '500, 500') scaled between 0 and 1000. Required for click actions.
        value: The text to type or the keyboard key to press (e.g. 'enter', 'win'). Required for type/key actions.
    """
...
CONFIG = {
    ...
    "system_instruction": "You are a helpful hands-free PC assistant. Limit your actions STRICTLY to: 1) 'click' (needs target [y, x]), 2) 'click_and_type' (needs target [y, x] and value to type), or 3) 'press_key' (needs exact pyautogui key value like 'enter', 'win', 'esc'). When the user asks you to interact with the screen or you need to see what is currently on the screen, use the request_screenshot tool. The image will be uploaded to your visual context immediately. Once you see the screen, you can use the execute_pc_action tool to carry out clicks or keystrokes based on normalized coordinates [y, x] scaled between 0 and 1000 (e.g. '500, 500' for the center). If an action fails, read the error message you get back, fix the error, and try again.",
